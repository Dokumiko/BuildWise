...
import { CompatibilityDisclaimer, FindingList, PowerSummary, visibleFindings } from "./evidence";
...
export function BuilderSection() {
  ...
  const actionableFindings = analysis
    ? visibleFindings(analysis.findings, selectedParts.map((part) => part.component_type))
    : [];
    ...
    <section className={styles.section} aria-label="Danh sách linh kiện và kết quả tương thích">
      ...
      <div className={styles.builderLayout}>
        ...
        <aside className={styles.analysisRail} aria-live="polite">
          ...
          {analysis && <>
            <PowerSummary summary={analysis.summary} />
            {actionableFindings.length > 0 && <>
              <p className={styles.status + " " + (errorCount > 0 ? styles.incompatible : styles.compatible_with_warnings)}>
                {errorCount > 0 ? "Có lỗi hoặc linh kiện không tương thích" : "Có điểm cần lưu ý"}
              </p>
              <p className={styles.muted}>{errorCount} lỗi · {warningCount} lưu ý</p>
